...
public class StixObservableContainer : Dictionary<StixIdentifier, CyberObservableObject>, IStixDataType //warn likely not implemented properly
{
...
public class ObservableContainerConverter : JsonConverter<StixObservableContainer>
{
    private static CyberObservableObject? ResolveElement(in JsonProperty element, in JsonSerializerOptions options)
    {
        ...
        Debug.WriteLine($"Skipped deserialisation of unknown {nameof(StixObservableContainer)}. {element.Value.GetRawText()}");
        return null;
    }

    public override StixObservableContainer? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
